...
*/	
	...
	void Update () {
		/*Debug.Log ("Last x value recieved:" + xPos);
		if (xPos >10 && xPos < 1000) { //do nothing
		}
		else if (xPos >= 1000)
			transform.Rotate (Vector3.up * 30 * Time.deltaTime); //go left
		else
			transform.Rotate (-Vector3.up * 30 * Time.deltaTime); //go right
